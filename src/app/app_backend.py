...
def infer_on_new_data(node_tensor,edge_tensor, edge_features,selected_option):
    ...
    '''
    model = get_model(selected_option)
    # Load checkpoint of trained model
    # checkpoint = torch.load(f'src/models/checkpoint_GBPre_{ model.__class__.__name__}.tar')
    checkpoint = torch.load("src/models/checkpoint_GBPre_GATe.tar")
    ...
    # Node features (100 nodes with 10 features each)
    x = torch.rand(num_nodes, num_features)

    # Edge index (connectivity for a graph with E edges)
    edge_index = torch.tensor([[0, 1, 2], [1, 2, 0]])  # Example edge connections

    # Edge attributes (E edges with edge_dim features each)
    edge_attr = torch.rand(edge_index.shape[1], edge_dim)

    # Run predictions
    with torch.no_grad(): predictions = model(x, edge_index, edge_attr)

    # Output predictions
    print(predictions)


    predictions = model(node_tensor,edge_tensor, edge_features)
