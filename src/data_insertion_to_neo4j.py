...
def load_demo_data():
    return pd.read_csv('../../data/LI-Small_Trans.csv', sep=',')
...
def insert_and_connect_data(tx, row_data):
    query = (
        ...
        MERGE (fb:Bank {id: $From_Bank})
        MERGE (tb:Bank {id: $To_Bank})
        MERGE (fa:Account {id: $From_Account})
        MERGE (ta:Account {id: $To_Account})

        // Create relationships
        CREATE (fb)-[:HAS_ACCOUNT]->(fa)
        CREATE (tb)-[:HAS_ACCOUNT]->(ta)
        CREATE (fa)-[:SENT]->(t)
        CREATE (t)-[:RECEIVED]->(ta)
        """
    )
